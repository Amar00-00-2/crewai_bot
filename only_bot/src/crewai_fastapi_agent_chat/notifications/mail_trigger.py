...
def send_python_email(emaildata:dict):
    ...
    <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4; text-align: center;">
        <div style="max-width: 600px; background: white; padding: 20px; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1); margin: auto;">
            <h2 style="color: #333;">🎉 Agent Email Sent Successfully! 🎉</h2>
    ...
    try:
        ...
        return f"✅ Email sent successfully to {emaildata['to']}."
    except requests.exceptions.RequestException as e:
        print(f"❌ Error sending email to {emaildata['to']}: {e}")
        return f"❌ Error sending email to {emaildata['to']}: {e}"
